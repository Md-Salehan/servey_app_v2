...
const LOVField = ({
  ...
  // Validation effect for required fields - FIXED: Only validate when modal is closed or on blur
  useEffect(() => {
    if (isModelOpened) {
      if (required && (!value || (multiple && Array.isArray(value) && value.length === 0))) {
  ...
  // Filter data based on search text - FIXED: Better search handling
  const filteredData = useMemo(() => {
    if (!searchText.trim() || !searchable) return localData;

    const searchLower = searchText.toLowerCase().trim();
    const searchKeysToUse = searchKeys.length > 0 ? searchKeys : columns.map(col => col.key);
  ...
  // Check if all items are selected - FIXED: Proper comparison
  const isAllSelected = useMemo(() => {
    ...
    const selectedKeys = selectedItems.map(item => {
      ...
      return typeof item === 'object' && item !== null ? item[primaryKey] : item;
    });
    
    return filteredData.every(item => 
      selectedKeys.includes(item[primaryKey])
    );
  }, [multiple, filteredData, selectedItems, primaryKey]);

  // Check if select all is disabled - FIXED: Added proper conditions
  const isSelectAllDisabled = useMemo(() => {
    if (!multiple) return true; // Select All only makes sense for multiple selection
    if (disabled) return true; // If the entire field is disabled, Select All should also be disabled
    if (filteredData.length === 0) return true; // No items to select
    if (maxSelections && selectedItems.length >= maxSelections && !isAllSelected) return true; // If max selections reached and not all are selected, disable Select All
    return false;
  }, [multiple, disabled, filteredData, maxSelections, selectedItems, isAllSelected]);

  // Get display text for trigger button - FIXED: Handle missing items
  ...
  }, [value, multiple, selectedItems, localData, displayKey, placeholder, primaryKey]);

  // Handle selection - FIXED: Proper state updates and close logic
  ...
  }, [multiple, selectedItems, maxSelections, onChange, disabled, primaryKey, closeOnSelect]);

  // Handle select all - FIXED: Properly handle partial selection
  const handleSelectAll = useCallback(() => {
    ...
    if (isAllSelected) {
      ...
      const newSelectedItems = selectedItems.filter(key => !visibleKeys.includes(key));
      setSelectedItems(newSelectedItems);
      onChange(newSelectedItems);
    } else {
  ...
  }, [multiple, disabled, filteredData, isAllSelected, selectedItems, maxSelections, onChange, primaryKey]);
  ...
  // Enhanced search with debounce - FIXED: Proper cleanup
  ...
  // Enhanced modal open with animations - FIXED: Easing import
  ...
  // Enhanced modal close with animations - FIXED: Proper cleanup
  ...
  // Render column headers - FIXED: Proper key handling
        ...
        {multiple && (
          <View style={styles.checkboxColumn}>
            <TouchableOpacity
              ...
              activeOpacity={0.6}
            >
              {isAllSelected && <Icon name="check" size={14} color={COLORS.surface} />}
            </TouchableOpacity>
          </View>
        )}
  ...
  // Default row renderer - FIXED: Proper item handling
  const defaultRenderRow = useCallback(({ item, index }) => {
        ...
        {multiple && (
          <View style={styles.checkboxColumn}>
            <View style={[
              ...
              {isItemSelected && <Icon name="check" size={14} color={COLORS.surface} />}
            </View>
          </View>
        )}
        {columns.map((column, colIndex) => {
          const cellValue = typeof item === 'object' ? item[column.key] : item;
          return (
            <View
              key={column.key}
              style={[
  ...
  }, [multiple, selectedItems, maxSelections, showClearAll, handleClearAll, disabled, showFooter]);

  // Render empty state with improved illustration
  const renderEmptyState = useCallback(() => (
    <View style={styles.emptyContainer}>
      <View style={styles.emptyIconContainer}>
        <Icon name={searchText ? 'search-off' : 'inbox'} size={56} color={COLORS.gray[300]} />
      </View>
  ...
  const renderSelectionBadge = useCallback(() => {
    if (!showSelectionCount || !multiple || selectedItems.length === 0) return null;
  ...
  if (isPreview) {
    ...
    const hasValue = value && (multiple ? selectedItems.length > 0 : value !== '');
      ...
      >
        <Text
          style={[
            ...
            (!value || (multiple && selectedItems.length === 0)) && commonStyles.datePickerPlaceholder,
                ...
                >
                  <View style={styles.modalHeader}>
                    <View style={styles.modalHeaderLeft}>
                      ...
                      {multiple && showSelectionCount && selectedItems.length > 0 && (
                        <View style={styles.headerBadge}>
                          <Text style={styles.headerBadgeText}>{selectedItems.length}</Text>
                        </View>
                      )}
                    </View>
                    <TouchableOpacity
                    ...
                    <View style={styles.selectAllContainer}>
                      ...
                      >
                        <View style={[
                          ...
                          {isAllSelected && <Icon name="check" size={12} color={COLORS.surface} />}
                        </View>
                  ...
                  <View style={styles.tableContainer}>
                    <FlatList
                      ...
                      ListHeaderComponent={filteredData.length > 0 ? renderColumnHeaders : null}
