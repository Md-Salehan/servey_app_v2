...
class UploadService {
  ...
  async uploadMultipleImages(images, formId, fcId, onProgress) {
    ...
    if (pendingConfirmations.length > 0) {
      const confirmResult = await this.confirmUploads(pendingConfirmations, formId, fcId);
  ...
  async retryFailedConfirmations(images, formId, fcId) {
    ...
    const confirmResult = await this.confirmUploads(failedConfirmations, formId, fcId);
