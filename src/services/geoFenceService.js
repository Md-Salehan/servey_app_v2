...
class GeoFenceService {
   ...
   */
  async saveGeoFenceData(appId, formId, userId, geojson, latitude, longitude) {
    console.log({appId, formId, userId, geojson, latitude, longitude}, "saveGeoFenceData");
      ...
      await this.database.write(async () => {
        if (existing.length > 0) {
          ...
          await geoFencesCollection.create(record => {
            record.fenceId = `fenceId_${appId}_${formId}_${userId}`;
            record.appId = appId;
   ...
   */
  async getGeoFenceData(appId, formId, userId) {
    try {
      const geoFencesCollection = this.database.collections.get('geo_fences');
   ...
   */
  async deleteGeoFenceData(appId, formId, userId) {
    try {
      const geoFencesCollection = this.database.collections.get('geo_fences');
      const existing = await geoFencesCollection
        .query(
